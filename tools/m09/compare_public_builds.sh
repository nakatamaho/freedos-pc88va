...
test -f "$2" && test -f "$3"
mkdir -p build
result=$(mktemp -d "$root/build/m09-public.XXXXXX")
image=${M09_BUILD_IMAGE:-freedos-pc88va-m01:local}
context=${M09_DOCKER_CONTEXT:-default}
...
for n in 1 2; do
  container=$("${docker_cmd[@]}" create --platform linux/amd64 --network none --user root --entrypoint bash \
    ...
    "$image" -ec '
      ...
      cd /work
      python3 tools/m09/rebuild_public.py --payload-dir /work/payload --output /output/media
    ')
...
echo 'M09 two clean public builds: objects, artifacts and canonical manifests byte-identical'
