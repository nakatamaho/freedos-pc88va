...
"""M15-owned FAT12/D88 primitives; no historical milestone runtime imports.

Project-authored routines retained from the M05 media and M14 readback
implementations at parent 5655861. Only the needed pure routines are included;
no historical acceptance metadata or build orchestration is consumed.
"""
...
class ValidationError(RuntimeError):
    """Raised for a bounded fail-closed M15 contract error."""
...
def derive_layout(spec: dict) -> dict:
    ...
    for key, value in declared.items():
        if derived[key] != value:
            raise ValidationError(f"declared M15 layout does not recompute: {key}")
...
def lba_to_chs(lba: int, geometry: dict) -> tuple[int, int, int]:
    ...
    if not isinstance(lba, int) or isinstance(lba, bool) or not 0 <= lba < total:
        raise ValidationError("LBA is outside the M15 geometry")
...
def build_boot_record(spec: dict) -> bytes:
    ...
    if placeholder != b"\xeb\xfe\x90":
        raise ValidationError("M15 placeholder must remain the documented self-loop")
    ...
    if filesystem_type != b"FAT12":
        raise ValidationError("M15 filesystem type label changed")
    sector[54:62] = filesystem_type.ljust(8, b" ")
    if sector[510:512] == b"\x55\xaa" or sector[1022:1024] == b"\x55\xaa":
        raise ValidationError("M15 boot record contains an undeclared firmware signature")
    return bytes(sector)
...
def parse_d88(d88_bytes: bytes, spec: dict, derived: dict) -> tuple[dict, bytes]:
    ...
    if sector_records != derived["total_sectors"] or len(raw) != derived["total_bytes"]:
        raise ValidationError("D88 sector set does not reconstruct the M15 geometry")
    return {
