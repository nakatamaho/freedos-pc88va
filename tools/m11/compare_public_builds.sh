...
# Two clean, network-disabled M11 builds using the accepted M01 container.
set -euo pipefail
root=$(git rev-parse --show-toplevel); cd "$root"
test "$#" = 3 || { echo 'usage: compare_public_builds.sh CHILD_SHA COMMAND.COM COUNTRY.SYS' >&2; exit 2; }
child=$1; [[ "$child" =~ ^[0-9a-f]{40}$ ]] || exit 2
test "$(git -C components/fdkernel rev-parse "$child^{commit}")" = "$child"
test -f "$2" -a -f "$3"
mkdir -p build
result=$(mktemp -d "$root/build/m11-public.XXXXXX")
image=${M11_BUILD_IMAGE:-${M10_BUILD_IMAGE:-freedos-pc88va-m01:local}}
context=${M11_DOCKER_CONTEXT:-${M10_DOCKER_CONTEXT:-default}}
...
for n in 1 2; do
  ...
  "${docker_cmd[@]}" cp "$2" "$container:/input/COMMAND.COM"; "${docker_cmd[@]}" cp "$3" "$container:/input/COUNTRY.SYS"
...
echo 'M11 two clean network-disabled builds: objects, artifacts and media are byte-identical'
echo "M11_BUILD_ROOT=$result"
